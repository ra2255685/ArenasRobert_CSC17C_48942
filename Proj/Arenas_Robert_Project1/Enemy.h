 ...
 *
 * Created on November 10, 2015, 3:41 PM
 */
...
void Enemy<T>::showEnemies(Enemy &obj){
    ...
    Node *nodePtr = obj.head; //pointer to object head
    ...
    if(!nodePtr){
        ...
        while(nodePtr){
           cout << nodePtr->name << " " << nodePtr->id << " has " << nodePtr->attack 
                   << " attack and is carrying " << nodePtr->gold << " gold" <<endl;
           nodePtr = nodePtr->next; //move pointer to next node
        }
    }
    cout << endl;
}
...
void Enemy<T>::selectEnemy(int n){
    ...
    if(head->id == n){
        ...
        while(nodePtr != NULL && nodePtr->id != n){
            nodePtr = nodePtr->next;
        }
        
        if(nodePtr){
            //set combat access stats for battle calculations
            eattack = nodePtr->attack;
            egold = nodePtr->gold;
            enemyName = nodePtr->name;
            eId = nodePtr->id;
        }else{
...
void Enemy<T>::sort(){
   Node *nodePtr = head;
   head = NULL;
   head = mergeSort(nodePtr); // returns list in ascending order
